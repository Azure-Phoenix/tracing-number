...
export class TexurePaint {
  ...
  mouse(key = "LEFT", context = document) {
    ...
    context.addEventListener("touchstart", (e) => {
      console.log(e)
      this.startPaint()
    })
    ...
    // if (!isMobile) {
      context.addEventListener("mousedown", (e) => {
        if (e.button == keyIndex) this.startPaint()
      })
      context.addEventListener("mouseup", (e) => {
        if (e.button == keyIndex) this.stopPaint()
      })
    // }
  }
  ...
  #draw(uv) {
    ...
    for (let i = 1; i < this.brush.s + 1; i++)
      for (let j = 1; j < this.brush.s + 1; j++) {
        ...
        backAlpha = this.#blendAlpha(
          this.buf[shift1 + 3],
          this.brush.buf[shift2] * this.brush.alpha
        )
        this.buf[shift1] = this.#blendColor(
          this.buf[shift1],
          this.brush.col.r * 255,
          this.buf[shift1 + 3],
          this.brush.buf[shift2] * this.brush.alpha,
          backAlpha
        )
        this.buf[shift1 + 1] = this.#blendColor(
          this.buf[shift1 + 1],
          this.brush.col.g * 255,
          this.buf[shift1 + 3],
          this.brush.buf[shift2] * this.brush.alpha,
          backAlpha
        )
        this.buf[shift1 + 2] = this.#blendColor(
          this.buf[shift1 + 2],
          this.brush.col.b * 255,
          this.buf[shift1 + 3],
          this.brush.buf[shift2] * this.brush.alpha,
          backAlpha
        )
